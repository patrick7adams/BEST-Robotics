...
#pragma config(Motor,  port4,           sweepServoTwo, tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port5,           sweepServoOne, tmotorVex393_MC29, openLoop)
...
task move { // Movement of the robot itself.
	...
	if(moveMode == 1) { // tank controls
		if(abs(vexRT[Ch3]) > moveLowerBound) {
			motor[leftMotor] = vexRT[Ch3]*moveMult;
		} else {
			motor[leftMotor] = 0;
		}
		if(abs(vexRT[Ch1]) > moveLowerBound) {
			motor[rightMotor] = vexRT[Ch1]*moveMult;
		} else {
			motor[rightMotor] = 0;
		}
	} else { // arcade controls
		if(abs(vexRT[Ch2]-vexRT[Ch1]) > moveLowerBound) {
			motor[leftMotor] = (vexRT[Ch2]-vexRT[Ch1])*moveMult;
		} else {
			motor[leftMotor] = 0;
		}
		if(abs(vexRT[Ch2]+vexRT[Ch1]) > moveLowerBound) {
			motor[rightMotor] = (vexRT[Ch2]+vexRT[Ch1])*moveMult;
		} else {
			motor[rightMotor] = 0;
		}
	} // end if
...
task servoSweep { // Pushes the arm servos whenever they are pressed.
	if(vexRT[Btn5U]) {
		motor[sweepServoOne] = -64;
		motor[sweepServoTwo] = 64;
	} else {
		motor[sweepServoOne] = 64;
		motor[sweepServoTwo] = -64;
	}
...
task arm { // Arm movement.
	if(moveMode == 2) { // Uses joystick if the robot is in arcade mode.
		if(abs(vexRT[Ch3]) > armLowerBound) {
			motor[armMotor] = vexRT[Ch3] * armMult;
		} else {
			motor[armMotor] = 0;
		}
	} else { // Uses buttons if the robot is in tank mode.
		if(vexRT[Btn8U]) {
			motor[armMotor] = 127 * armMult;
		} else if (vexRT[Btn8D]) {
			motor[armMotor] = -127 * armMult;
		} else {
			motor[armMotor] = 0;
